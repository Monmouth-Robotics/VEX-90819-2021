...
void runProgSkills() {
	...
	pidForward(0, 24, 0, 80, 0.25, 50, 20, 20, 0, 0, 0, 0, 0, 0, true);
	pidTurn(3*M_PI/2-M_PI/4, 80, 0.025, 120.0, 0.0, 0.0);
	printf("Angle: %.3f\n", position.getTheta()*180/M_PI);
	intakeMotorLeft = 127;
	intakeMotorRight = 127;
	pidForward(-16, 8, 3*M_PI/2-M_PI/4, 63, 0.5, 50, 15, 50, 0, 0, 0, 0, 0, 0, true);
	upperStack = 127;
	lowerStack = 127;
	pros::delay(1000);
	upperStack = 0;
	...
	pidForward(-10, 24, M_PI*3/2, 63, 0.5, 50, 50, 20, 0, 0, 0, 0, 0, 0, true);
